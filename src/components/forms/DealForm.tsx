// src/components/customers/CustomerSearchSelect.tsx
...
export default function CustomerSearchSelect({
  ...
  // Load full selected if we have an id but no selected object
  useEffect(() => {
    let cancelled = false;
    if (!value) {
      setSelected(null);
  ...
  return (
    <div className={className} ref={boxRef}>
      ...
      {selected?.id ? (
        <p className="mt-1 text-xs text-neutral-500">Selected: {selected.name || selected.email} — {selected.id}</p>
      ) : null}
    </div>
  );
}
