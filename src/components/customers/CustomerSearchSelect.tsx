...
/** ✅ Named export so DealForm can `import { StripeCustomerLite } ...` */
export type StripeCustomerLite = { id: string; name: string; email: string };

type Props = {
  /** Selected Stripe customer id */
  value?: string | null;
  /** Emits selected id and a lite record; pass `null` to clear */
  onChange: (id: string | null, lite?: StripeCustomerLite | null) => void;
  placeholder?: string;
  className?: string;
};

/** ✅ Default export: NO references to DealForm/Props, no self-imports */
export default function CustomerSearchSelect({
  ...
  const displayValue = useMemo(() => {
    ...
    if (!selected) return "";
    return selected.name || selected.email || selected.id;
  }, [query, selected]);
